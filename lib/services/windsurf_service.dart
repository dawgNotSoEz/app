...
class WindsurfService extends ChangeNotifier {
  // In a real app, this would come from an API
  List<WindsurfSpot> getSpots() {
    try {
      // Using the fromJson constructor to demonstrate proper usage
      return _getSpotData().map((spotData) => WindsurfSpot.fromJson(spotData)).toList();
    } catch (e) {
      // Error handling
      print('Error fetching windsurf spots: $e');
      return _getFallbackSpots();
    }
  }

  // This simulates data coming from an API
  List<Map<String, dynamic>> _getSpotData() {
    return [
      {
        'id': '1',
        'name': 'Tarifa',
  ...
  // Fallback spots in case of error
  List<WindsurfSpot> _getFallbackSpots() {
    return [
      WindsurfSpot(
        id: '1',
        name: 'Tarifa',
        location: 'Spain',
        imageUrl: 'assets/images/placeholder.txt', // Fallback to local asset
        rating: 4.8,
        difficulty: 'Advanced',
        conditions: {
          'wind': '15-35 knots',
        ...
        description: 'Tarifa is known as the wind capital of Europe.',
      ),
      WindsurfSpot(
        id: '2',
        name: 'Maui',
        ...
        description: 'Maui is a world-renowned windsurfing destination.',
      ),
    ];
  }
}
